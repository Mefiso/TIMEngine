...
public:
	...
	void SendEvent(const SDL_Event& _event) const;		// Processes all ImGui events
	void CreateViewport();								// Sends the definition (FBO, RBO, colorBuffer) of the viewport from ModuleRender to W_Viewport
	void Log(const char* _input) const;					// Sends LOG inputs from the log function to W_Console
